...
const EMPLOYER_ROUTES = {
    ...
    // Update these routes to point to Dashboard with Company Branding tab
    announcement_approved: (roid) => ({ path: DASHBOARD_PATH, state: { targetTab: "Company Branding", targetAnnouncementId: roid } }),
    announcement_rejected: (roid) => ({ path: DASHBOARD_PATH, state: { targetTab: "Company Branding", targetAnnouncementId: roid } }),
    announcement_submitted: (roid) => ({ path: DASHBOARD_PATH, state: { targetTab: "Company Branding", targetAnnouncementId: roid } }),
};
...
const JOBSEEKER_ROUTES = {
    ...
    new_message: (roid) => ({ path: `/Job-portal/jobseeker/chat/`, state: { conversationId: roid }, }),
    announcement_published: (roid) => ({ path: "/Job-portal/jobseeker/announcements", state: { targetAnnouncementId: roid } }),
